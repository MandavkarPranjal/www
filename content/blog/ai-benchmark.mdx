...
Manually adding context to chat session is not a good practice, cause us human make errors/mistakes which may lead to missing context. But if LLMs has right tool to add context itself or to search whole codebase(*it will not do it, unless u write shit code*) if want to, it will output a good code
